...
def load_vllm_model(
    ...
    Args:
        ...
        sampling_seed: Global vLLM random seed used for sampling
...
def evaluate_math500(
    ...
    if dataset_name.lower() == "math500":
        local_path = Path(__file__).parent.parent / "data" / "eval" / "MATH-500" / "test.parquet"
        if local_path.exists():
            dataset = load_dataset("parquet", data_files=str(local_path), split="train")
            print(f"Loaded local MATH-500 dataset from {local_path} with {len(dataset)} problems")
        else:
        ...
        local_path = Path(__file__).parent.parent / "data" / "aime24" / "test.jsonl"
        if local_path.exists():
            dataset = load_dataset("json", data_files=str(local_path), split="train")
            print(f"Loaded local AIME24 dataset from {local_path} with {len(dataset)} problems")
        else:
            dataset = load_dataset("HuggingFaceH4/aime_2024", split="train")
            print(f"Loaded HuggingFaceH4/aime_2024 dataset with {len(dataset)} problems")
    elif dataset_name.lower() == "aime25":
        local_path = Path(__file__).parent.parent / "data" / "aime25"
        if local_path.exists():
            dataset = load_dataset("json", data_files=str(local_path / "test.jsonl"), split="train")
            print(f"Loaded local AIME25 dataset from {local_path} with {len(dataset)} problems")
        else:
        ...
        local_path = Path(__file__).parent.parent / "data" / "hmmt25_feb"
        if local_path.exists():
            dataset = load_dataset("json", data_files=str(local_path / "test.jsonl"), split="train")
            print(f"Loaded local HMMT25 dataset from {local_path} with {len(dataset)} problems")
        else:
    ...
    total_length = 0  # Total tokens generated
    ...
    for idx, (output, problem, gt_answer, question_id) in enumerate(
        ...
        # Record generation length in tokens
    ...
    format_rate = formatted_count / total * 100
    avg_length = total_length / total  # Average generation length in tokens
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Explicit vLLM sampling seed used for paired reproducibility (default: 0)",
    )
    ...
    print(f"Max concurrent sequences: {args.max_num_seqs}")
    print("=" * 70 + "\n")
